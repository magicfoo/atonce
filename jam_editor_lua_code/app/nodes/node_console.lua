...
function createView( inWindowType, inSettings )
	if inWindowType == 'console' then
		local av = new 'MFlConsoleView'
...
function onMessage ( fa, uri, m )
	...
	if m.subject == 'console-out' then
		if m.content[1] then
			for k,v in pairs( views ) do
				node.handle.facade:postMessage( {subject='private-add-output', target=k,content={m.content[1]} } )
			end
		end
	end
end
