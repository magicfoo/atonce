...
#ifndef _su_sg_coreframe_h_
#define _su_sg_coreframe_h_
...
namespace su { namespace sg
{
	...
	struct Frame: public Core
	{
		static const uint Type = 0xef0350b8;		// su::sg::Frame

		static Frame*	Create		(	Handle*			inHandle	);
		
		void			SetPosition	(	const Vec3&		inPos		);
		void			SetRotation	(	const Quat&		inRot		);
		void			SetTR		(	const Matrix&	inTR		);
